...
exports.createExecutivo = async (executivos) => {
  ...
  executivos.forEach(exec => {
    ...
    batch.set(ref, { nome: exec.nome, sobrenome: exec.sobrenome, email: exec.email, apelido: exec.apelido || "" });
    refs.push({ ref, ...exec });
  });
  await batch.commit();
...
exports.updateExecutivo = async (id, { nome, sobrenome, email, apelido }) => {
  await db.collection(COLLECTION_NAME).doc(id).update({ nome, sobrenome, email, apelido });
  return { id, nome, sobrenome, email, apelido };
};
